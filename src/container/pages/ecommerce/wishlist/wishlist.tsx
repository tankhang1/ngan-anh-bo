  ...
  function handleDelete(id: string) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const updatedInvoiceData = allData.filter(
          (idx) => idx.id.toString() !== id
        );
            ...
            <Card className="custom-card product-card">
              <Card.Body className="d-sm-flex align-items-center flex-wrap">
                <a
                  ...
                  className="whishlist-image mb-2"
                >
                ...
                <div className="ms-3 mt-2 mt-sm-0">
                  <h5 className="product-name fs-16 fw-semibold mb-0 align-items-center">
                    {idx.title}
                  </h5>
                  <p className="product-description fs-13 text-muted mb-0">
                    {idx.description}
                  </p>
                  <h4 className="mb-1 fw-semibold">
                    <span>
                      {idx.newpr}
                      <span className="text-muted text-decoration-line-through ms-1  fs-14 op-6">
                        {idx.oldpr}
                      </span>
                    </span>
                  </h4>
                  <p
                    className={`badge bg-${idx.color}-transparent fs-11 rounded-1 mb-0 me-1`}
                  >
                    {idx.offerprice}
                  </p>
                  <span className="badge bg-warning-transparent rounded-1 me-1">
                    4.2{" "}
                  ...
                  <div className="mt-2">
                    <Link
                      ...
                      className="d-inline-flex btn btn-sm btn-info-light me-2 mb-1"
                      onClick={() => {
                        AddToCart(idx.id);
                      }}
                    >
                      <i className="ri-shopping-cart-line me-2"></i>Move To Cart
                    </Link>
                    <Link
                      to={`${
                        import.meta.env.BASE_URL
                      }pages/ecommerce/productdetails`}
                      className="d-inline-flex btn btn-sm  btn-success-light mb-1"
                    >
                      <i className="ri-eye-line me-2"></i>View Product
                    </Link>
                  </div>
                </div>
              </Card.Body>
      ...
      <div className="float-end mb-4">
        <nav aria-label="Page navigation" className="pagination-style-4">
          <Pagination className=" mb-0 gap-2">
            ...
            <Pagination.Item className="d-none d-sm-flex" href="#">
              17
            </Pagination.Item>
